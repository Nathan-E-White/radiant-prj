...
`RunConnectionProfile` already renders Kubernetes namespace and Job name fields, but no SimOps Kubernetes adapter exists under `backend/slurm-gateway`, and the only `k8s.io/client-go` import found in this checkout is in the sandbox runtime. Source: `backend/slurm-gateway/internal/gateway/simops_run_connection_profile.go:53-56`, `backend/slurm-gateway/internal/gateway/simops_run_connection_profile.go:157-160`, `sandbox/randiant-runtime/libs/k8s-runtime/kube_runtime.go:14-19`.

The sandbox runtime is reference material rather than integrated SimOps runtime: its README describes a separate orchestrator with Docker, K8s, and monitor commands, and its K8s flow uses Kind plus OpenTofu and destroys a cluster it created. Source: `sandbox/randiant-runtime/README.md:1-15`, `sandbox/randiant-runtime/README.md:17-30`.

The sandbox K8s runtime uses client-go to list Pods and stream Pod logs, but its OpenTofu manifest creates a `kubernetes_pod_v1`, not a Kubernetes Job. Source: `sandbox/randiant-runtime/libs/k8s-runtime/kube_runtime.go:158-220`, `sandbox/randiant-runtime/infra/tofu/k8s/main.tf:15-37`.
...
| #25 Kind/client-go E2E proof | Not present for SimOps. Kind/client-go exists only in sandbox reference code. | <https://github.com/Nathan-E-White/radiant-prj/issues/19#issuecomment-4933153528>, `sandbox/randiant-runtime/README.md:1-30`, `sandbox/randiant-runtime/libs/k8s-runtime/kube_runtime.go:14-19` |
| #26 OpenTofu substrate lane | Not present as the PRD lane. Existing top-level Tofu is placeholder custom Docker infra; sandbox Tofu applies a Pod, not static SimOps namespace/RBAC/service-account substrate. | <https://github.com/Nathan-E-White/radiant-prj/issues/19#issuecomment-4933153528>, `infra/main.tofu:3-29`, `sandbox/randiant-runtime/infra/tofu/k8s/main.tf:15-37` |
...
Kind/Kubernetes should follow Docker because the production gateway does not yet accept a Kubernetes runtime, while sandbox Kind/client-go code is separate reference material. Source: `backend/slurm-gateway/internal/gateway/config.go:440-464`, `sandbox/randiant-runtime/README.md:1-30`.

OpenTofu should remain substrate-only because issue #19 excludes per-run OpenTofu applies, and the current repo has no finished SimOps namespace/RBAC/service-account substrate lane. Source: <https://github.com/Nathan-E-White/radiant-prj/issues/19>, `infra/main.tofu:3-29`, `sandbox/randiant-runtime/infra/tofu/k8s/main.tf:15-37`.
...
There is a naming and expectation risk around the existing sandbox: it demonstrates Kind/client-go/OpenTofu mechanics, but it provisions Pods and helper logs rather than implementing the SimOps client-go Job adapter required by issue #19. Source: `sandbox/randiant-runtime/README.md:5-12`, `sandbox/randiant-runtime/infra/tofu/k8s/main.tf:15-37`, <https://github.com/Nathan-E-White/radiant-prj/issues/19>.
...
Kubernetes planning should treat the sandbox as reference evidence only and design a production SimOps adapter around native Jobs, labels, ConfigMap/Secret references, service account, restart policy, and cleanup TTL from `RunConnectionProfile`. Source: `sandbox/randiant-runtime/libs/k8s-runtime/kube_runtime.go:158-220`, `sandbox/randiant-runtime/infra/tofu/k8s/main.tf:15-37`, `backend/slurm-gateway/internal/gateway/simops_run_connection_profile.go:53-60`, <https://kubernetes.io/docs/concepts/workloads/controllers/job/>.
