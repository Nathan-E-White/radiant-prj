...
| Repository manifest tests | `bun run repository:manifest:test`, included by `bun run repository:verify:test` | Focused executable assertions for repository-claim registration and controlled-document inventory |
...
| Capability Ledger manifest tests | `bun run capability:ledger:manifest:test`, included by `bun run capability:ledger:test` | Focused executable assertions for Ledger-to-claim, surface, and controlled-document linkage |
...
| Initial capability reconciliation baseline | `bun run capability:reconcile -- HEAD` at the recorded revision | [Controlled Issue #179 closeout record](capability-reconciliation-baseline.md) with classified findings and disposition |
...
| Pipeline policy | `bun run pipeline:policy:check` | Declared least-privilege, untrusted pull-request, trusted-publication, and required-command policy evaluated against workflow adapters and Ledger records |
...
| Workbench dataflow Snapshot parser | `bun run simulator-workbench:dataflow:json:test` | Valid coherent Snapshot acceptance and partial/generation-mismatch rejection |
