...
Issues and PRDs for this repo live as GitHub issues in `Nathan-E-White/radiant-prj`. Use the `gh` CLI for all operations.
...
- **Frontier query**: list the map's open children with `gh issue list --state open`, scoped to the map's sub-issues or task list; drop any with an open blocker or an assignee; first in map order wins.
