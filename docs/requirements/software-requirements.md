...
| SW-026 | The architecture shall govern cross-store delivery, Run lifecycle, stream observation, telemetry admission, and Workbench review as at-least-once, identity-led, and reconciled contracts without claiming cross-store exactly-once processing. | Retries and presentation composition must preserve Delivery Assurance, Artifact Forge, Experience Scenario, Run, and Snapshot truth boundaries. | Configuration audit and targeted tests | Accepted architecture contract; implementation evidence pending |
