...
| SR-005 | Configuration audit | Controlled quality, design, verification, and release document set | `bun run quality:check` | QP-011 |
| SR-006 | Configuration audit | WIP, fold-back, and v2 checkpoint script dry-runs | Script dry-runs | REL-001 |
...
| SW-005 | Test | Documentation quality checker | `bun run quality:check` | VVR-001 |
| SW-006 | Configuration audit | Release operation scripts with dry-run modes | Script dry-runs | REL-001 |
...
7. Run `bun run quality:check` and confirm the v2 controlled document package is complete.
