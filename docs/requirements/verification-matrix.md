...
| DEV-PACKAGING-001 | Test and analysis | Root build-context budget, reproducible Bun lock, dependency-free mock worker, narrow Go role images, cache budgets, and raw/gzip browser-asset budgets | `bun run docker:packaging:contract:check`, `bun run docker:packaging:verify`, `bun run test:mutation:docker-packaging` | `docs/verification/docker-packaging-evidence.json` |
...
| SW-026 | Configuration audit and targeted tests | Recoverable delivery, Run, stream, telemetry, and review-truth architecture contract; later slices add behavior evidence at their highest existing seams | `bun run repository:verify -- --claim architecture.issue-202-contract`; targeted delivery, Artifact Forge, lifecycle, stream, telemetry, flush, and review-context tests after implementation | RDRRT-001; implementation evidence pending |
...
13. Run `bun run simops:smoke:docker-orbstack` and confirm the Docker/OrbStack runtime proof reports API launch, gateway-only ingest, read-only observed Docker lifecycle, explicit success cleanup, failed-run retention with logs, and forced cleanup. Use `SIMOPS_SMOKE_BUILD=always` when the evidence must include a fresh image rebuild.
